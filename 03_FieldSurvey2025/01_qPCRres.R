...
library(rphylopic); packageVersion("rphylopic") # 1.3.0
...
d_sur <- d %>% filter(sur_bot == "surface") # Currently exclude the OP samples
...
g2 <- ggmap(lt_map2) +
  geom_phylopic(aes(x = 113.8530, y = 22.27928), fill = "pink2", color = "gray20", alpha = 0.75, img = img2, size = 0.005) +
  geom_phylopic(aes(x = 113.8530, y = 22.27728), fill = "gray", color = "gray20", alpha = 0.75, img = img2, size = 0.003) +
  geom_phylopic(aes(x = 113.8281, y = 22.22888), fill = "pink2", color = "gray20", alpha = 0.75, img = img2, size = 0.005) +
  geom_point(data = d, aes(x = lon, y = lat), color = "red3", shape = 4) +
  xlab(expression(paste("Longitude (", degree, "E)"))) +
  ylab(expression(paste("Latitude (", degree, "N)"))) +
  geom_text_repel(data = d, aes(x = lon, y = lat, label = sample_name), max.overlaps = 25) +
...
g7 <- ggmap(lt_map2) +
  ...
  xlab(expression(paste("Longitude (", degree, "E)"))) +
  ylab(expression(paste("Latitude (", degree, "N)"))) +
  ggtitle("Summer survey in 2025.1") +
  NULL
...
# ------------------------------------------ #
# Save Robj
saveRDS(list(g5,g6,g7), "../04_FigCode/data_robj/Fig_Survey_winter.obj")

# Save figures
ggsave("01_qPCRresOut/LantauWinter_map.pdf", g1, width = 10, height = 8)
ggsave("01_qPCRresOut/LantauWinter_map_CWD.pdf", g2, width = 10, height = 8)
ggsave("01_qPCRresOut/qPCR_detection_winter.pdf", g3 + g4, width = 15, height = 8)
ggsave("01_qPCRresOut/qPCR_detection_winter_comb.pdf", g7, width = 9, height = 8)
ggsave("01_qPCRresOut/qPCR_concs_winter.pdf", g5 + g6, width = 15, height = 8)
