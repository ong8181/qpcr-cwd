...
(outdir <- outdir_create())

## Read in your data file (MODIFY FILE NAME AS NEEDED):
DAT <- read.csv("data_lod/data_std.csv") # <---- User-specified

## Remove outliers
remove_outlier <- FALSE
if (remove_outlier) DAT <- DAT[!DAT$outlier,]

## Define your CV threshold for LoQ:
LOQ.Threshold <- 0.35 # <---- User-specified

## Define which logarithmic function to use for LoD model:
LOD.FCT <- "Best" # <---- User-specified
...
write(paste0("Analysis started: ",date(),"\n\n"),file=sprintf("%s/Analysis Log.txt", outdir))
...
if(sum(colnames(DAT) == "Target")!=1) { #Is there a "Target" column?
  ...
  if(length(A)!=1) { write("There is a problem with the 'Target' column.\n\n",file=sprintf("%s/Analysis Log.txt", outdir),append=TRUE) } #Add error message to analysis log.
...
if(sum(is.na(DAT$SQ))>0) {
  write(paste0("WARNING: ",sum(is.na(DAT$SQ))," data points excluded without a valid starting quantity (SQ)!\nHere is a sample of the data being excluded:\n"),
        file=sprintf("%s/Analysis Log.txt", outdir),append=TRUE)
  suppressWarnings(write.table(head(DAT[is.na(DAT$SQ),]),file=sprintf("%s/Analysis Log.txt", outdir),append=TRUE,
...
for(i in 1:length(Targets)) {
  ...
  if(length(unique(DAT$SQ[DAT$Target==Targets[i]]))!=length(STDS2)) {
    ...
    write(ToWrite,file=sprintf("%s/Analysis Log.txt", outdir),append=TRUE)
  }
...
write("Data Summary:",file=sprintf("%s/Analysis Log.txt", outdir),append=TRUE)
suppressWarnings(write.table(DAT2,file=sprintf("%s/Analysis Log.txt", outdir),append=TRUE,sep="\t",eol="\n",
                             row.names=FALSE,col.names=TRUE))
...
write("Assay summary:",file=sprintf("%s/Analysis Log.txt", outdir),append=TRUE)
write("\nR.squared: The R-squared value of linear regression of all standards Cq-values vs log10 of the starting quantities.",file=sprintf("%s/Analysis Log.txt", outdir),append=TRUE)
write("Slope: The slope of the linear regression.",file=sprintf("%s/Analysis Log.txt", outdir),append=TRUE)
write("Intercept: The y-intercept of the linear regression.",file=sprintf("%s/Analysis Log.txt", outdir),append=TRUE)
write("\nLow.95: The lowest standard with at least 95% positive detection.",file=sprintf("%s/Analysis Log.txt", outdir),append=TRUE)
write("LOD: The 95% limit of detection as determined by probit modeling.",file=sprintf("%s/Analysis Log.txt", outdir),append=TRUE)
write(paste0("LOQ: The limit of quantification as determined by decay modeling, using the user-selected CV threshold of: ",LOQ.Threshold),file=sprintf("%s/Analysis Log.txt", outdir),append=TRUE)
write("\nrep2.LOD: The effective limit of detection if analyzing in 2 replicates.",file=sprintf("%s/Analysis Log.txt", outdir),append=TRUE)
write("rep3.LOD: The effective limit of detection if analyzing in 3 replicates.",file=sprintf("%s/Analysis Log.txt", outdir),append=TRUE)
write("rep4.LOD: The effective limit of detection if analyzing in 4 replicates.",file=sprintf("%s/Analysis Log.txt", outdir),append=TRUE)
write("rep5.LOD: The effective limit of detection if analyzing in 5 replicates.",file=sprintf("%s/Analysis Log.txt", outdir),append=TRUE)
write("rep8.LOD: The effective limit of detection if analyzing in 8 replicates.\n\n",file=sprintf("%s/Analysis Log.txt", outdir),append=TRUE)
write.csv(DAT3,file=sprintf("%s/Assay summary.csv", outdir),row.names=FALSE)
  ...
  ggOut <- ggplot(data=DAT[DAT$Target==Targets[i]&is.na(DAT$SQ)==FALSE,],
    ...
    annotate("text",y=min(DAT$Cq[DAT$Target==Targets[i]&is.na(DAT$SQ)==FALSE],na.rm=TRUE)*1.05,
             ...
             label=(paste0("R-squared: ", round(DAT3$R.squared[i], 3),
                           ...
                           "\nPCR efficiency: ", round((10^(1/(-DAT3$Slope[i]))-1)*100, 1), "%"))) +
    scale_x_log10(breaks = c(1, 10, 100, 1000, 10000, 100000, 1000000), label= label_10_to_power)
  print(ggOut)
...
for(i in 1:length(Targets)) {
  if(!is.na(LOD.list2[i+1])) {
    ...
    mtext(paste0("FCT used: ",LOD.list3[i+1],"    Lack of fit test: p = ", round(Pval, 3)),side=3)
  }
  if(is.na(LOD.list2[i+1])) {
...
write.csv(LOD.CI,file=sprintf("%s/LOD_confint.csv", outdir),row.names=FALSE)
...
for(i in 1:length(Targets)) {
  ...
  Decay.Plot <- ggplot(DAT2[DAT2$Target==Targets[i],], aes(x= Standards, y = Cq.CV)) +
    ...
    ylab("Coefficient of variation for Ct values") +
    xlab("Standard concentrations (Copies / Reaction)") +
...
# ------------------------------------------------ #
i <- 1
mylod_mod <- get(LOD.list2[i+1])
# Calculate predictions
x_min <- min(DAT4[,1:4], na.rm = TRUE)
x_max <- max(DAT$SQ, na.rm = TRUE)
log_x_vals <- seq(log10(x_min), log10(x_max), length.out = 100)
x_vals <- 10^log_x_vals
preds <- data.frame(conc = x_vals,
                    prob = predict(mylod_mod, newdata = data.frame(conc = x_vals)))
# Replicate values
rep_names <- c("Limit of Detection","2 Replicates LoD","3 Replicates LoD",
               "4 Replicates LoD","5 Replicates LoD","8 Replicates LoD")
rep_df <- data.frame(conc=DAT4[1:LODS,1], prob=YS[1:LODS], col = rep_names)
rep_df$col <- factor(rep_df$col, levels = rep_names)
# 95% CI
ci_df <- data.frame(conc = DAT4$Estimate, prob = YS[1:LODS],
                    xmin = DAT4$Lower, xmax = DAT4$Upper,
                    col = rep_names)

# Draw using ggplot2
g4 <- ggplot(preds, aes(x = conc, y = prob)) +
  geom_line(color = "gray10") +
  geom_vline(xintercept=DAT3$LOD[1],colour="red") +
  geom_errorbarh(data = ci_df, aes(xmin = xmin, xmax = xmax, y = prob, color = col), height = 0.02) +
  scale_color_manual(values = cols4all::c4a("seaborn.bright"), name = NULL) +
  geom_point(data = rep_df, aes(x = conc, y = prob, color = col, shape = col), size = 1.5) +
  scale_color_manual(values = cols4all::c4a("seaborn.bright"), name = NULL) +
  scale_shape_manual(values = c(16,17,15,3,7,8), name = NULL) +
  labs(x = "Standard concentrations (Copies / Reaction)",
       y = "Detection Probability") +
  scale_x_log10(label = label_10_to_power) +
  theme_bw() +
  NULL
...
# Save plot
# ------------------------------------------------ #
g1 <- ggOut + scale_x_log10(label = label_10_to_power)
g2 <- Decay.Plot + scale_x_log10(label = label_10_to_power) + theme_bw()

#dir.create("01_LoD-calculatorOut")
ggsave("01_LoD-calculatorOut/LOD.pdf", g1, width = 9, height = 6)
ggsave("01_LoD-calculatorOut/DecayPlot.pdf", g2, width = 9, height = 6)

# Edit ggOut
ggOut2 <- g1
ggOut2$layers[[2]] <- NULL
ggOut2$layers[[6]] <- NULL
ggOut2$layers[[7]] <- NULL
g3 <- ggOut2 +
  #geom_abline(slope = -3.097, intercept = 38.94, linetype = 3) +
  #geom_abline(slope = -3.327, intercept = 39.86, linetype = 1) +
  labs(subtitle = expression(paste("R"^2, ": AA, y = -AAA x + AA.AA, PCR efficiency = AA.A%"))) +
  theme(plot.subtitle = element_text(hjust = 0.5)) +
  scale_x_log10(label = label_10_to_power, breaks = c(1, 10, 100, 1000, 10000, 100000))

## Save ggOut2
ggsave("01_LoD-calculatorOut/LOD_edited.pdf", g3, width = 9, height = 6)

## Save RObject
saveRDS(list(g1,g2,g3,g4), "../04_FigCode/data_robj/Fig_LOD_LOQ.obj")
